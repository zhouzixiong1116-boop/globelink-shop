...
app.get("/api/events", (req, res) => {
  ...
  req.on("close", () => { clearInterval(heartbeat); const idx = clients.indexOf(res); if (idx > -1) clients.splice(idx, 1); });
  clients.push(res);
});

function broadcast(type, payload) {
  const data = `data: ${JSON.stringify({ type, payload, ts: Date.now() })}\n\n`;
  for (const client of clients) { try { client.write(data); } catch (_) {} }
}
...
function initData() {
  ...
  const of = path.join(DATA_DIR, "orders.json");
  ...
  if (!fs.existsSync(pf)) {
    writeJSON(pf, [
      ...
      { id: 2, name: "电脑不限速", category: "vpn", price: 88, spec: "月付 88元/1000G", tags: ["教程","电脑","不限速"], description: "电脑端不限时高速加速，月流量1000G。", status: "active" },
      ...
      { id: 6, name: "美区Apple ID（永久）", category: "apple", price: 58, spec: "独享绑定", tags: ["教程","永久","独享"], description: "永久独享账号，支持绑定个人信息。", status: "active" },
      { id: 7, name: "Apple家庭组", category: "apple", price: 268, spec: "6人团 年度", tags: ["教程","家庭组"], description: "加入Apple家庭组，共享付费应用和资源。", status: "active" },
      { id: 8, name: "GPT Plus 会员", category: "charge", price: 35, spec: "月付", tags: ["代充","秒到"], description: "ChatGPT Plus月卡代充。", status: "active" },
      { id: 9, name: "Claude Pro", category: "charge", price: 40, spec: "月付", tags: ["代充","秒到"], description: "Anthropic Claude Pro月度会员代充。", status: "active" },
      { id: 10, name: "Midjourney", category: "charge", price: 30, spec: "月付", tags: ["代充","AI绘画"], description: "MJ月度会员代充。", status: "active" },
      { id: 11, name: "远程配置服务", category: "extra", price: 8, spec: "一对一指导", tags: ["远程","教程"], description: "一对一远程协助。", status: "active" },
      { id: 12, name: "其他国外软件代充", category: "charge", price: 20, spec: "按需报价", tags: ["代充","咨询"], description: "其他国外软件也可代充，请联系客服询价。", status: "active" },
    ]);
  }
  if (!fs.existsSync(of)) { writeJSON(of, []); }
  if (!fs.existsSync(sf)) {
    writeJSON(sf, {
...
app.get("/api/admin/products", (req, res) => {
  res.json(readJSON(path.join(DATA_DIR, "products.json"), []));
});

app.post("/api/admin/products", (req, res) => {
  const products = readJSON(path.join(DATA_DIR, "products.json"), []);
  const { name, category, price, spec, tags, description, status } = req.body;
  if (!name || !category || price === undefined) { return res.status(400).json({ error: "缺少必要字段" }); }
  const maxId = products.reduce((max, p) => Math.max(max, p.id), 0);
  const product = { id: maxId + 1, name, category, price, spec: spec || "", tags: tags || [], description: description || "", status: status || "active" };
...
app.delete("/api/admin/products/:id", (req, res) => {
  const products = readJSON(path.join(DATA_DIR, "products.json"), []);
  const id = parseInt(req.params.id);
  writeJSON(path.join(DATA_DIR, "products.json"), products.filter(p => p.id !== id));
...
app.post("/api/orders", (req, res) => {
  ...
  if (!customerName || !contact || !payment) { return res.status(400).json({ error: "缺少必要字段" }); }
  const order = { id: "GL" + Date.now(), product_id: productId || null, product_name: productName || "", price: price || 0, customer_name: customerName, contact: contact, payment: payment, note: note || "", status: "pending", created_at: new Date().toLocaleString("zh-CN") };
...
app.get("/api/admin/orders", (req, res) => { res.json(readJSON(path.join(DATA_DIR, "orders.json"), [])); });
...
app.patch("/api/admin/orders/:orderId/status", (req, res) => {
  ...
  if (!["pending", "completed", "cancelled"].includes(status)) { return res.status(400).json({ error: "无效的状态值" }); }
...
app.delete("/api/admin/orders/:orderId", (req, res) => {
  const orders = readJSON(path.join(DATA_DIR, "orders.json"), []);
  const orderId = req.params.orderId;
  writeJSON(path.join(DATA_DIR, "orders.json"), orders.filter(o => o.id !== orderId));
  broadcast("order_deleted", { id: orderId });
  res.json({ success: true });
});

app.get("/api/settings", (req, res) => { res.json(readJSON(path.join(DATA_DIR, "settings.json"), {})); });
